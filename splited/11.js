          ...
          <div class="card shadow-sm product-card" onclick="showProductDetails(${product.id})">
            <div style="position: relative;">
              ...
              <button class="wishlist-btn position-absolute top-0 end-0 m-2" 
                ...
                <i class="bi bi-heart-fill" style="color: ${isInWishlist ? 'red' : '#ccc'};"></i>
              </button>
            </div>
            <div class="card-body">
              <h5 class="card-title">${product.name}</h5>
...
function showProductDetails(productId) {
  ...
  const initialImage = product.colorImages[defaultColor] || (product.images && product.images.length ? product.images[0] : 'https://via.placeholder.com/400x300?text=' + encodeURIComponent(product.name));
  ...
  let colorThumbnails = colors.map(color => {
    const imageUrl = product.colorImages[color] || initialImage;
    return `
  ...
  const detailContent = `
    <div class="container">
      <div class="row">
        <div class="col-md-6">
          <img id="mainProductImage" src="${initialImage}" 
               class="product-detail-img" alt="${product.name}" 
               style="width: 100%; height: auto; cursor: pointer;" 
               onclick="showProductImagesModal(${product.id})">
...
// Update the generateProducts function to handle featured products
function generateProducts(containerId, productList) {
  const container = document.getElementById(containerId);
  
  if (!container) return;
  
  // For featured products, we'll show all without count
  if (containerId !== "featuredProductsContainer") {
    const countElement = document.getElementById(`${containerId.replace("Container", "")}ResultsCount`);
    if (countElement) {
      const query = document.getElementById("searchInput")?.value.trim();
  ...
  productList.forEach(product => {
    ...
    // For featured products, use a slightly different layout
    if (containerId === "featuredProductsContainer") {
      const card = `
        <div class="col-6 col-md-4 col-lg-3 mb-4">
          <div class="card shadow-sm product-card" onclick="showProductDetails(${product.id})">
      ...
      // Default layout for other containers
      const card = `
        <div class="col-6 mb-4">
          <div class="card shadow-sm product-card" style="position: relative;" onclick="showProductDetails(${product.id})">
            <img src="${product.images && product.images.length ? product.images[0] : 'https://via.placeholder.com/400x300?text=' + encodeURIComponent(product.name)}" 
